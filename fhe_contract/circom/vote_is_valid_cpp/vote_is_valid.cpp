...
void vote_is_valid_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void vote_is_valid_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 50;}
...
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 11],&signalValues[mySignalStart + 12]); // line circom 32
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
}
...
vote_is_valid_4_create(1,0,ctx,"main",0);
vote_is_valid_4_run(0,ctx);
}
